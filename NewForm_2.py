...
for filez in filesindir:
	...
	#Repeating variables
	cnty = raw_excel.iat[0,0].encode('utf8').title()
	repEnt = raw_excel.iat[1,0].encode('utf8').title()
	repEntType = raw_excel.iat[2,0].encode('utf8').title()
	...
	if len(good_match) > 1:
		anstorepEnt = raw_input('No match, enter name for '+ repEnt + '\n')
		print (anstorepEnt)
