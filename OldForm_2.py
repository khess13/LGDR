...
def error_handle_for_csv(df_to_test, NOF):
	if df_to_test.empty:
		...
		cnty = raw_input('Fix county \n').encode('utf8')
		repEntType = raw_input('Fix RepEntType \n').encode('utf8')
		repEnt = raw_input('Fix RepEnt \n').encode('utf8')
		#try call again
...
for filez in filesindir:
	...
	#Repeating variables
	cnty = raw_excel.iat[0,0].encode('utf8').title().strip()
	repEnt = raw_excel.iat[1,0].encode('utf8').title().strip()
	fiscYr = pd.to_datetime(raw_excel.iat[3,0], \
			 format = '%d%b%Y').strftime('%m/%d/%Y')
	repEntType = raw_excel.iat[2,0].encode('utf8').title().strip()
	...
	if len(good_match) > 1:
		anstorepEnt = raw_input('No match, enter name for '+ cnty + ' ' +
															repEntType + ' ' +
															repEnt + '\n')
		print (anstorepEnt)
		...
		if yesno == 'N' or yesno == 'n':
			anstorepEnt = raw_input('Okay, try again \n' + repEnt)
		#if no answer given
		...
		if len(good_match) > 1:
			print (good_match)
			yesno = raw_input('Yes to skip this file?  Y/N')
