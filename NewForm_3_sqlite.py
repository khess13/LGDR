...
def Send_To_DB (tablename,df_send):
	#Baka test
	try:
		sql.to_sql(df_send, name=tablename, con=conn, if_exists='append')
	except:
		print ('Somethings wrong with this file: ',filez)

'''END FUNCTIONS'''

'''DB Housekeeping'''
sqlpath = inputpath+'data.sqlite'
...
for filez in filesindir:
	...
	ContactInfo = gendata[['County','ReportingEntity','ReportingEntityType',
						'FiscalYear','PreparedBy','Email','Phone','Fax','FileDate']]
	...
	'''Send to DB'''
	Send_To_DB('Debt', Debt)
	Send_To_DB('CountyStatistics', CountyStatistics)
	Send_To_DB('ContactInfo', ContactInfo)
